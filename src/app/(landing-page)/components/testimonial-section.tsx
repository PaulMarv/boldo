...
export default function TestimonialSection() {
    const [testimonialsOrder, setTestimonialsOrder] = useState(testimonial);

    const handleNext = () => {
        setTestimonialsOrder((prev) => {
            const nextOrder = [...prev];
            const firstItem = nextOrder.shift();
            nextOrder.push(firstItem!);
            return nextOrder;
        });
    };
    ...
    const handleBack = () => {
        setTestimonialsOrder((prev) => {
            ...
            const lastItem = prevOrder.pop();
            prevOrder.unshift(lastItem!);
            return prevOrder;
        });
    };
                ...
                <div className='lg:px-[150px] px-[20px]'>
                    <div className='flex lg:flex-row lg:items-end justify-between flex-col'>
                        ...
                        <div className='hidden gap-3 lg:flex lg:mt-0 mt-5 justify-center lg:justify-start'>
                            <span
                                className='rounded-full bg-white flex items-center justify-center lg:h-[72px] w-[36px] lg:w-[72px] h-[36px] cursor-pointer'
                                onClick={handleBack}
                            >
                    ...
                    <div className='lg:mt-[74px] mt-[37px] grid grid-cols-1 lg:grid-cols-3 lg:justify-between justify-items-center lg:gap-[50px] gap-6'>
                        {testimonialsOrder.map(testimony => (
                            // Testimonial card
